...
def demand_rule(model,r,f,b,t):
	...
	for p in model.fix_pilots :
		if (p, r, f, b) in model.fix_var_set:
			vfixp +=model.V_position[p, r, f, b, t]
	...
	for p in model.nonfix_pilots :
		if (p, r, f, b) in model.nonfix_var_set:
			nonfix_pilot +=model.Yall[p, r, f, b, t]
...
def pilot_transit_rule1(model, p, r, f, b, t):
	return model.Yall[p,r,f,b,t] - model.Yall[p,r,f,b,t+1] >= 0
...
def trainee_rule2(model,p,r,f,b,t):
	if p in model.fleet_pilots:
		if(t >= 2):
			return model.Trainee_pos[p,r,f,b,t] >= model.Trainee[p,b,t] + model.Trainee[p,b,t-1] + model.Trainee[p,b,t-2] +model.Yall[p,r,f,b,t] -1
		elif(t >= 1):
			return model.Trainee_pos[p,r,f,b,t] >= model.Trainee[p,b,t] + model.Trainee[p,b,t-1]+model.Yall[p,r,f,b,t] -1
		else:
			return model.Trainee_pos[p,r,f,b,t] >= model.Trainee[p,b,t] + model.Yall[p,r,f,b,t] -1
	else:
		return pe.Constraint.Skip
...
def trainee_var_binding_rule(model, p, r, f, b, t):
	if p in model.fleet_pilots:
		return model.Yall[p,r,f,b,t] - model.Yall[p,r,f,b,t+1] - model.Trainee[p, b, t+1] == 0
	else:
		return pe.Constraint.Skip
...
def vacation_position_rule(model,p,r,f,b,t):
	lhs = 0
	lhs = model.V[p,t] + model.Yall[p,r,f,b,t] - model.V_position[p,r,f,b,t] - 1
	return lhs <= 0
...
###OBJECTIVE FUNCTION###
##Transition costs:
model.total_fleet_trans_cost = pe.summation(model.fleet_transition_cost, model.Yall, index = [(p, r, f, b, model.endtime) for(p, r, f, b) in model.to_pos if p in model.fleet_pilots ])
model.total_base_trans_cost = pe.summation(model.base_transition_cost, model.Yall, index = [(p, r, f, b, model.endtime) for(p, r, f, b) in model.to_pos if p in model.base_pilots ])
...
for (p, r, f, b) in model.nonfix_var_set:
	for t in model.time:
		...
		if((p in model.base_pilots) & (t in model.timestart) & ((p,r,f,b) in model.from_pos)):
			if((model.Yall[p, r, f, b, t].value == 1) & (model.Yall[p, r, f, b, t+1].value == 0)):
