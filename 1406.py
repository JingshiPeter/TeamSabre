...
model.train_start_time = pe.Set(initialize=range(24))
...
model.Yonwork = pe.Var(model.all_var_set*model.time, domain=pe.Binary)
...
#def trainer_rule_onwork(model,p,b,t):
#	rhs = 0
#	for f in model.fleet:
#		for r in model.rank:
#			if (p,r,f,b) in model.all_var_set:
#				rhs=rhs+model.Yonwork[p,r,f,b,t]
#	return rhs <= model.T[p,b,t]
#model.trainer_constraintononwork = pe.Constraint(model.trainer_pilots*model.base*model.time,rule=trainer_rule_onwork)
##

def pilot_on_work1(model, p, r, f, b,t):
    return  model.Yonwork[p,r,f,b,t] <= model.Yall[p,r,f,b,t]
...
#def trainee_rule_onwork(model,p,b,t):
#	rhs=0
#	for f in model.fleet:
#		for r in model.rank:
#			if (p,r,f,b) in model.nonfix_var_set:
#				rhs=rhs+model.Yonwork[p,r,f,b,t]	
#	return rhs <= model.Trainee[p,b,t]
#model.trainee_constraint_onwork = pe.Constraint(model.fleet_pilots*model.base*model.time,rule=trainee_rule_onwork)
...
def training_rule_onwork(model,p,r,f,b,t):
	if (p,r,f,b) in model.all_var_set:
		return model.Yonwork[p,r,f,b,t] <= model.Tposition[p,r,f,b,t]
	else:
		return pe.Constraint.Skip
...
def trainee_rule2(model,p,r,f,b,t):
	if (p,r,f,b) in model.all_var_set:
		if(t >= 2):
			return model.Trainee_po[p,r,f,b,t] >= model.Trainee[p,b,t] + model.Trainee[p,b,t-1] + model.Trainee[p,b,t-2] +model.Y[p,r,f,b,t] -1
		elif(t >= 1):
			return model.Trainee_po[p,r,f,b,t] >= model.Trainee[p,b,t] + model.Trainee[p,b,t-1]+model.Y[p,r,f,b,t] -1
		else:
...
def trainee_rule2_onwork(model,p,r,f,b,t):
	if (p,r,f,b) in model.all_var_set:
		return  model.Yonwork[p,r,f,b,t] <= model.Trainee_po[p,r,f,b,t]
	else:
		return pe.Constraint.Skip
...
def vacation_position_rule_onwork(model,p,r,f,b,t):
	return model.Yonwork[p,r,f,b,t] <= model.Vposition[p,r,f,b,t]
...
def vacation_position_rule2_onwork(model,p,r,f,b,t):
	return model.Yonwork[p,r,f,b,t] <= model.Vfix_position[p,r,f,b,t]
...
model.operationminus = pe.summation(model.dailycost,model.Yonwork)

model.OBJ = pe.Objective(expr = model.total_shortage_cost + model.total_trans_cost + model.total_normal_cost + model.total_vacation_penalty + model.operationcost - model.operationminus - model.total_seniority_reward, sense=pe.minimize)
